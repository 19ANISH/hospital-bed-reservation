...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/without'
db=SQLAlchemy(app)
...
class Hospitaldata(db.Model):
    ...
    HosAdd = db.Column(db.String(250))
    HosPhone = db.Column(db.String(12))
...
def addHospitalInfo():
    ...
    code = posts.HosCode
    # print(code)
    #postsdata = Hospitaluser.query.filter_by(HosCode=code).first()
    postsdata = Hospitaldata.query.filter_by(HosCode=code).first()
    if request.method=="POST":
        HosCode = request.form.get('HosCode')
        HosName = request.form.get('HosName')
        HosAdd = request.form.get('HosAdd')
        HosPhone = request.form.get('HosPhone')
        ...
        if huser:
            db.engine.execute(f"INSERT INTO `hospitaldata` (`HosCode`,`HosName`,`HosAdd`,`HosPhone`,`normalbed`,`icubed`,`ventbed`) VALUES ('{HosCode}','{HosName}','{HosAdd}','{HosPhone}','{normalbed}','{icubed}','{ventbed}')")
...
def bedbooking():
    ...
    #query2  = db.engine.execute(f"SELECT * FROM `hospitaluser`")
    if request.method == 'POST':
        email = request.form.get('email')
        HosCode = request.form.get('HosCode')
        ...
        dbb1 = db.engine.execute(f"SELECT `HosName`, `HosAdd`, `HosPhone` FROM `hospitaldata` WHERE `HosCode` = '{code}'")
        x1 = dbb1.fetchall()
        # for dbb1 in x1[0]:
        #     print(ddb1)
        #print(x1)
        ...
        #print(x1[0][1])
        #print(x1[0][2])
        mail2 = db.engine.execute(f"SELECT `email` FROM `hospitaluser` WHERE `hospitaluser`.`HosCode` = (SELECT `HosCode` FROM `hospitaldata` WHERE `hospitaldata`.`HosCode` = '{code}')")
        x = mail2.fetchall()
        ...
        mail_check = Hospitaluser.query.filter_by(HosCode=HosCode).first()
        #print(mail_check)
        ...
        if (seat > 0 and check2):
            ...
            mail.send_message("Welcome", sender=params['admin-email'], recipients=[email],
                              body=f"Thanks for booking\nDetails\nEmail Address used:- {email}\nHospital Name:- {x1[0][0]}\nHospital Code:- {HosCode}\nHospital Address:- {x1[0][1]}\nHospital Phone:- {x1[0][2]}\nBed Type:- {bedtype}\nPatient Name:- {pname}\nPatient Contact:- {pphone}\n\nContact hospital as soon as possible\n\n\t\tThank You")
            #mail.send_message("Welcome", sender=params['admin-email'], recipients=[dbe],body=f"Works fine")
            # for dbb1 in x1[0]:
            #     flash(f"Bed Booked,Save The details {dbb1}", "success")
            for mail2 in x[0]:
                mail.send_message("Welcome", sender=params['admin-email'], recipients=[mail2],
                                  body=f"Patient Details\nEmail Address used:- {email}\nHospital Code:- {HosCode}\nBed Type:- {bedtype}\nPatient Name:- {pname}\nPatient Contact:- {pphone}\n\nThey will contact you as soon as possible\n\n\t\tThank You")
                flash("Mail Sent To Hospital Please Contact", "success")
...
#     return render_template("All.html",query=query)

if __name__ == '__main__':
    app.run(debug=True)  # to run the flask app

#required query
#SELECT email FROM hospitaluser WHERE hospitaluser.HosCode = (SELECT HosCode FROM hospitaldata WHERE hospitaldata.HosCode = "H101");