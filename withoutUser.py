...
def hedit(id):
    ...
    if request.method=='POST':
        ...
        ventbed = request.form.get('ventbed')
        # HosCode = HosCode.upper()
        db.engine.execute(f"UPDATE `hospitaldata` SET `HosCode` = '{HosCode}', `HosName` = '{HosName}', `normalbed` = '{normalbed}', `icubed` = '{icubed}', `ventbed` = '{ventbed}' WHERE `hospitaldata`.`id`={id}")
